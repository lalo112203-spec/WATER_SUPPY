        ...
        <div
            ...
            <form method="POST" action="{{ route('billing.update', $bill) }}" class="relative z-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-200 mb-2 ml-1">Customer</label>
                            <input type="text" readonly value="{{ $bill->customer->name }} ({{ $bill->customer->customer_id }})" 
                                class="w-full bg-[#1b2636]/40 border border-[#2d4059] text-gray-200 text-sm rounded-xl py-3 px-4 outline-none">
                            <input type="hidden" id="customer_id" name="customer_id" value="{{ $bill->customer_id }}">
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="billing_date" class="block text-sm font-medium text-gray-200 mb-2 ml-1">Billing Date</label>
                            ...
                            <div>
                                <label for="due_date" class="block text-sm font-medium text-gray-200 mb-2 ml-1">Due Date</label>
                    ...
                    <div class="flex flex-col justify-between">
                        <div class="space-y-6 bg-[#0f1722]/40 p-6 rounded-2xl border border-[#263548]">
                             <div class="grid grid-cols-1 gap-5">
                                <div>
                                    <label for="calculated_usage_display" class="block text-xs font-medium text-gray-200 mb-1 ml-1">Calculated Consumption (m³)</label>
                                ...
                                <div>
                                    <label for="base_charge" class="block text-xs font-medium text-gray-200 mb-1 ml-1">Base Charge</label>
                                ...
                                <div>
                                    <label for="usage_charge" class="block text-xs font-medium text-gray-200 mb-1 ml-1">Usage Charge</label>
                                    <div class="relative text-gray-300 font-medium">
                                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-200">₱</span>
                        ...
                        <div class="flex gap-4 mt-8">
                            <button type="submit" id="edit_bill_btn" class="flex-1 bg-amber-600 hover:bg-amber-500 text-white font-bold py-4 rounded-2xl shadow-[0_4px_15px_rgba(217,119,6,0.4)] transition-all flex items-center justify-center gap-2">
                                Save Changes
                            </button>
                            <a href="{{ route('billing.show', $bill) }}" class="px-6 bg-[#1b2636]/60 text-gray-200 rounded-2xl flex items-center border border-[#2d4059]">Cancel</a>
                        </div>
                    </div>
                </div>
            </form>
