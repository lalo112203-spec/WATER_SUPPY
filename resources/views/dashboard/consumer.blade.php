    ...
    <div class="h-full flex flex-col py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto font-sans">
        ...
        <div class="grid grid-cols-1 gap-8 flex-1 mb-8 mx-2 items-start">
            ...
            <div class="bg-[#121a25]/80 backdrop-blur-md shadow-[0_4px_20px_rgb(0,0,0,0.03)] rounded-3xl overflow-hidden border border-[#263548]">
                <div class="p-6 border-b border-[#263548] flex items-center justify-between">
                ...
                <div class="overflow-x-auto w-full">
                    <table class="w-full text-left border-collapse min-w-full">
                        <thead class="bg-[#0f1722]/80">
                            <tr>
                                <th class="px-6 py-4 text-[12px] font-bold text-gray-400 uppercase tracking-widest border-b border-[#263548]">Date</th>
                                <th class="px-6 py-4 text-[12px] font-bold text-gray-400 uppercase tracking-widest border-b border-[#263548]">Usage</th>
                                <th class="px-6 py-4 text-[12px] font-bold text-gray-400 uppercase tracking-widest border-b border-[#263548]">Total</th>
                                <th class="px-6 py-4 text-[12px] font-bold text-gray-400 uppercase tracking-widest border-b border-[#263548]">Status</th>
                                <th class="px-6 py-4 text-[12px] font-bold text-gray-400 uppercase tracking-widest border-b border-[#263548] text-right">Action</th>
                            </tr>
                        </thead>
                            ...
                            @if(isset($customer) && $customer->bills->count() > 0)
                                @foreach($customer->bills as $bill)
                                    <tr class="hover:bg-blue-50/30 transition-colors">
                                        <td class="px-6 py-5 whitespace-nowrap text-[15px] font-semibold text-gray-200 tracking-tight">{{ \Carbon\Carbon::parse($bill->billing_date)->format('M d, Y') }}</td>
                                        <td class="px-6 py-5 whitespace-nowrap text-[15px] font-medium text-gray-400">{{ $bill->usage_units }} <span class="text-xs">L</span></td>
                                        <td class="px-6 py-5 whitespace-nowrap text-[15px] font-bold text-gray-100">₱{{ number_format($bill->total_amount, 2) }}</td>
